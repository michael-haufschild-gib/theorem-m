...
standard axioms. The sequence $(M_k)$ is provably \emph{not} a
P\'olya--Schur multiplier sequence --- no nondegenerate moment
sequence is, since moment sequences are log-convex while multiplier
sequences must be log-concave --- so the hyperbolicity proved here is
a property of the pair (sequence, family) and is not subsumed by any
universal coefficientwise preserver theorem. To our knowledge $\Psi_d$
is, in this generality, a new explicit hyperbolic deformation of the
Laguerre--Hermite family.
\end{abstract}
...
damped by a different factor. Moreover, $(M_k)$ is provably
\emph{not} a multiplier sequence in the sense of P\'olya--Schur:
already the image of $(w^2-1)^2$ under the coefficientwise action has
four nonreal zeros (Proposition~\ref{prop:notms}), so no
universal coefficientwise preserver theorem for all even hyperbolic
polynomials can yield
Theorem~\ref{thm:main} --- the hyperbolicity below is a property of
the pair (sequence, family), not of the sequence alone.
...
Section~\ref{sec:lean} describes the formalization; the complete
source, with build instructions and the verification scripts, is
available at
\begin{center}
\url{https://github.com/michael-haufschild-gib/theorem-m}
\end{center}
...
hyperbolicity?} The question is intrinsically tied to the family: no
nondegenerate moment sequence preserves hyperbolicity of \emph{all}
even hyperbolic polynomials (Remark~\ref{rem:logconvex} --- moment
sequences are log-convex, multiplier sequences must be log-concave),
so family-specific structure is the only possible source of such
preservation. The compound-Poisson mechanism of
...
sharpness and a proof that $(M_k)$ is \emph{not} a P\'olya--Schur
multiplier sequence, so that no universal coefficientwise preserver
theorem subsumes Theorem~\ref{thm:main}.
...
concentrated on $(0,1]$ with total mass $1-p$, and for every $d\ge 1$ the
...
$p$ at $1$, so $\mu=\nu-p\delta_1\ge 0$ is concentrated on $(0,1]$
with mass $1-p$ (in fact $\mu(\{1\})=0$, since the atom of $\nu$ at
$1$ has mass exactly $p$; nothing below uses this, and we do not
record it formally). Finally, for any $d\ge 1$,
...
\texttt{c5ea0035}\,/\,tag v4.30.0).\footnote{Source, build
instructions and verification scripts:
\url{https://github.com/michael-haufschild-gib/theorem-m}.} The
development comprises thirteen files, about $5{,}300$ lines:
...
obtained as a pushforward --- with $\mu\ge0$, concentration on
$(0,1]$, mass $1-p$ and even moments $M_k-p$. What it does not do is represent $U$ as the
...
(\texttt{nanoda\_lib}, commit \texttt{f58f2f6d}), an independent
implementation of the Lean~4 kernel in
Rust,\footnote{\url{https://github.com/leanprover/lean4export} and
\url{https://github.com/ammkrn/nanoda_lib}.} which reports
...
preserves hyperbolicity for every $d$. It is natural to ask whether
$(M_k)_{k\ge0}$ is a P\'olya--Schur multiplier sequence --- whether it
preserves hyperbolicity of \emph{all} hyperbolic even polynomials,
not just this family. The answer is no, by a one-line certificate.

\begin{proposition}[$(M_k)$ is not a multiplier sequence]\label{prop:notms}
The coefficientwise action
$\sum_k a_k w^{2k}\mapsto\sum_k a_k M_k w^{2k}$ does not preserve
hyperbolicity: the image of the hyperbolic polynomial $(w^2-1)^2$ is
$M_2 w^4 - 2M_1 w^2 + 1$, all four of whose zeros are nonreal.
\end{proposition}

\begin{proof}
The zeros satisfy $w^2=\bigl(M_1\pm\sqrt{M_1^2-M_2}\,\bigr)/M_2$, and
by \eqref{eq:Mk}, $M_0=1$, $M_1=\e^{\gamma}/2$ and
$M_2=\tfrac34\e^{2\gamma-1}$, so
\[
M_1^2-M_0M_2
\;=\;\frac{\e^{2\gamma}}{4}\Bigl(1-\frac{3}{\e}\Bigr)\;<\;0
\qquad\text{because }\e<3 .
\]
Hence $w^2$ is nonreal, and a nonreal number has no real square root.
\end{proof}

\begin{remark}[moment sequences vs.\ multiplier sequences]\label{rem:logconvex}
The failure is structural, not an accident of \eqref{eq:Mk}. Applying
the coefficientwise action of any sequence $(\gamma_k)$ with
$\gamma_k>0$ to the hyperbolic polynomials
$w^{2k-2}(w^2-1)^2$ for each $k\ge1$ shows that an even multiplier
sequence must satisfy the Tur\'an inequalities
$\gamma_k^2\ge\gamma_{k-1}\gamma_{k+1}$, i.e.\ must be
\emph{log-concave}. But every moment sequence
$\gamma_k=\E[V^k]$ of a nonnegative random variable is
\emph{log-convex} by Cauchy--Schwarz, strictly at $k=1$ unless $V$ is
a.s.\ constant. So no nondegenerate moment sequence is a multiplier
sequence --- the two classes meet only in the geometric sequences
$\gamma_k=c^k$, whose action is the rescaling $w\mapsto\sqrt{c}\,w$.
In particular, no P\'olya--Schur-type universal coefficientwise
preserver theorem could prove Theorem~\ref{thm:main}: what the theorem
certifies is a property of the \emph{pair} (sequence, family), powered
by the energy structure of $C_d$ (Lemma~\ref{lem:energy}) and the atom
bound $p>\tfrac12$ --- and by Proposition~\ref{prop:notms} that pairing
is essential. We regard this as making Theorem~\ref{thm:main} more
...
DOI: \url{https://doi.org/10.1515/crll.1914.144.89}.

\bibitem{dlmf}
NIST Digital Library of Mathematical Functions,
\url{https://dlmf.nist.gov/},
Release 1.2.6 of 2026-03-15; F.~W.~J. Olver,
A.~B. Olde Daalhuis, D.~W. Lozier, B.~I. Schneider,
R.~F. Boisvert, C.~W. Clark, B.~R. Miller,
...
DOI: \url{https://doi.org/10.1007/978-3-030-79876-5_37}.
...
DOI: \url{https://doi.org/10.1145/3372885.3373824}.
