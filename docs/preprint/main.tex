...
at every finite $d$; no limit is involved.
...
The family $\Psi_d$ arises in a research program around the Riemann
$\xi$-function, as the fixed (degree-independent) moment deformation
of the critical Laguerre profile; the moments \eqref{eq:Mk} are the
``hard head'' coefficients of that program's critical model lift, and
Theorem~\ref{thm:main} establishes the hyperbolicity of the model
anchor at every finite degree. We emphasize the scope plainly:
\textbf{Theorem~\ref{thm:main} proves nothing about the Riemann
hypothesis}. Whatever interest the theorem has should rest on its
self-standing content: an explicit, nontrivially deformed Laguerre
family that remains hyperbolic at every degree, with an elementary,
machine-checked proof and an exact probabilistic mechanism
\eqref{eq:decomp} behind it.
...
sharpness and on what is \emph{not} proved here.
...
\texttt{Defs} & the polynomials $\Psi_d$, $C_d$, the sequences $M_k$, $S_1$; statement of the theorem\\
...
\texttt{CriticalData} & Lemma~\ref{lem:critical} for $C_d$ via the bridge \eqref{eq:bridge}\\
...
\emph{No randomness in the formal text.} The formal proof does not
construct a Poisson point process. It constructs the finite measure
$\lambda$ directly, proves the integer Binet identity as a genuine
Lebesgue-integral statement, and obtains the residual measure $\mu$ of
\eqref{eq:decomp} as a pushforward, discharging a finite list of
moment conditions ($\mu\ge0$, mass $1-p$, even moments $M_k-p$ for
$k\le d$) --- which is all that Step~1 consumes. The probabilistic
language of Section~\ref{sec:moments} is, for the formal text, a
commentary.
...
each other's proofs), with the Lean kernel as the final referee. An
independent re-verification of the compiled proof term with an
external Lean~4 kernel checker is in progress and will be reported in
a revision.
...
Theorem~\ref{thm:main} is a statement about a fixed, explicit
polynomial family. It neither implies nor is implied by any statement
about the Riemann $\xi$-function; in the research program where
$\Psi_d$ arose, the passage from finite-degree hyperbolicity to any
statement about $\xi$ is a separate, open transport problem, and
nothing in this paper bears on it. We state this explicitly to
