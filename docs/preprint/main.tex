...
coefficientwise preserver theorem. To our knowledge $\Psi_d$ is, in
this generality, a new explicit hyperbolic deformation of the
...
coefficientwise preserver theorem can yield
...
multiplier sequence, so that no coefficientwise preserver theorem
subsumes Theorem~\ref{thm:main}.
...
by \eqref{eq:Mk}, $M_1=\e^{\gamma}/2$ and $M_2=\tfrac34\e^{2\gamma-1}$,
so
\[
M_1^2-M_0M_2
...
$\gamma_k>0$ to the hyperbolic polynomials $w^{2k-2}(w^2-1)^2$ shows
that an even multiplier sequence must satisfy the Tur\'an inequalities
...
In particular, no coefficientwise preserver theorem could prove
Theorem~\ref{thm:main}: what the theorem certifies is a property of
the \emph{pair} (sequence, family), powered by the energy structure of
$C_d$ (Lemma~\ref{lem:energy}) and the atom bound $p>\tfrac12$ --- and
by Proposition~\ref{prop:notms} that pairing is essential. We regard
this as making Theorem~\ref{thm:main} more interesting, not less.
\end{remark}
